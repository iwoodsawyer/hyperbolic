function [R,Q,J] = jrq(A,J,econ,tol)
%JRQ J-orthogonal (or J-unitary, or hyperbolic) RQ factorization
%   [R,Q] = JRQ(A,J) computes a J-orthogonal (or J-unitary, or hyperbolic)
%   RQ factorization of A such that matrix A = R*Q, where R is an upper
...
%
%   [R,Q,Jp] = JRQ(A,J) returns a permuted J-orthogonal, which is the case
%   when row permutations are applied for the hyperbolic rotations. If row
%   permutations are applied, than Q*J*Q' = Jp.
%
%   [R,Q,Jp] = JRQ(A,J,0) produces the "economy size" decomposition.
%
%   [R,Q,Jp] = JRQ(A,J,[],tol) is an optional tolerance parameter can be
...
if nargin < 1
    error('JRQ requires at least two input arguments')
end
                ...
                if J(r1) == J(r2) % same sign2, apply orthogonal rotation
                    ...
                    if abs(da) > tol*(abs(a1)+abs(a2))
                        ...
                        if (r2 > 1 || retry > 0) % still entries remaining
                            ...
                            error('JRQ failed to apply hyperbolic rotation. Check tolerance or method.')
                        end
                    end
                end
            end
                ...
                if J(r1) == J(r2) % same sign2, apply orthogonal rotation
                    ...
                    if abs(da) > tol*(abs(a1)+abs(a2))
                        ...
                        if (r2 > 1 || retry > 0) % still entries remaining
                            ...
                            error('JRQ failed to apply hyperbolic rotation. Check tolerance.')
                        end
                    end
                end
            end
