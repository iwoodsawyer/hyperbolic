function [Q,R,J] = jqr(A,J,econ,tol)
%JQR J-orthogonal (or J-unitary, or hyperbolic) QR factorization
%   [Q,R] = JQR(A,J) computes a J-orthogonal (or J-unitary, or hyperbolic)
%   QR factorization of A such that matrix A = Q*R, where R is an upper
...
%
%   [Q,R,Jp] = JQR(A,J) returns a permuted J-orthogonal, which is the case
%   when row permutations are applied for the hyperbolic rotations. If row
%   permutations are applied, than Q'*J*Q = Jp.
%
%   [Q,R,Jp] = JQR(A,J,0) produces the "economy size" decomposition.
%
%   [Q,R,Jp] = JQR(A,J,[],tol) is an optional tolerance parameter can be
...
if nargin < 1
    error('JQR requires at least two input arguments')
end
                ...
                if J(r1) == J(r2) % same sign2, apply orthogonal rotation
                    ...
                    if abs(da) > tol*(abs(a1)+abs(a2))
                        ...
                        if (r2 < m || retry > 0) % still entries remaining
                            ...
                            error('JQR failed to apply hyperbolic rotation. Check tolerance or method.')
                        end
                    end
                end
            end
                ...
                if J(r1) == J(r2) % same sign2, apply orthogonal rotation
                    ...
                    if abs(da) > tol*(abs(a1)+abs(a2))
                        ...
                        if (r2 < m || retry > 0) % still entries remaining
                            ...
                            error('JQR failed to apply hyperbolic rotation. Check tolerance.')
                        end
                    end
                end
            end
