function [R,Q,J] = jlq(A,J,econ,tol)
%JLQ J-ortogonal (or J-unitary, or hyperbolic) LQ factorization
%   [L,Q] = JLQ(A,J) computes a J-orthogonal (or J-unitary, or hyperbolic)
%   LQfactorization of A such that matrix A = L*Q, where L is an lower
...
%
%   [L,Q,Jp] = JLQ(A,J) returns a permuted J-orthogonal, which is the case
%   when row permutations are applied for the hyperbolic rotations. If row
%   permutations are applied, than Q*J*Q' = Jp.
%
%   [L,Q,Jp] = JLQ(A,J,0) produces the "economy size" decomposition.
%
%   [L,Q,Jp] = JLQ(A,J,[],tol) is an optional tolerance parameter can be
...
if nargin < 1
    error('JLQ requires at least two input arguments')
end
...
if size(J,1) == size(J,2)
    if size(J,1) ~= n
        error('J must have size(A,2)*size(A,2)')
    end
    J = sign2(diag(J)); % keep only diagonal entries
else
    if length(J) ~= n
        error('J must have size(A,2) entries')
    end
    J = sign2(J);
end
if any(J==0)
...
if isreal(R) % real Givens rotations
    while c < min(m,n)
        ...
        retry = 0;
        r2_saved = [];
        while (r2 < n || retry > 0) % cancel entries in the column
            if r2 < n % proceed to cancel entries
                r2 = r2+1;
            ...
            if abs(a2) <= tol*abs(a1)
                ...
                if J(r1) == J(r2) % same sign2, apply orthogonal rotation
                    ...
                    if abs(da) > tol*(abs(a1)+abs(a2))
                        ...
                        if (r2 < n || retry > 0) % still entries remaining
                            % cancelling all remaining entries first
                            retry = retry + 1;
                            r2_saved = [r2 r2_saved];
                            
                        else % no hyperbolic rotation
                            error('JLQ failed to apply hyperbolic rotation. Check tolerance or method.')
                        end
                    end
                end
            end
                ...
                if J(r1) == J(r2) % same sign2, apply orthogonal rotation
                    ...
                    if abs(da) > tol*(abs(a1)+abs(a2))
                        ...
                        if (r2 < n || retry > 0) % still entries remaining
                            ...
                            error('JLQ failed to apply hyperbolic rotation. Check tolerance.')
                        end
                    end
                end
            end
